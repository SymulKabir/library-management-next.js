...
const ActionBookMenu = ({
  book_id,
  setBooks,
}: {
  book_id: string;
  setBooks: (fn: any) => void;
}) => {
  const [processing, setProcessing] = useState<{ delete: boolean }>({
    delete: false,
  });
  const router = useRouter();

  const handleEdit = () =>
    router.push(`/admin/dashboard/inventory/edit/${book_id}`);
  const apiCall = async () => {
    try {
      setProcessing((state) => {
        return {
      ...
      setBooks((books: any[]) =>
        books.filter((book) => book.book_id !== book_id),
      );
      return null;
    } catch (error: any) {
  ...
  const delete_book = async () => {
    ...
    try {
      const result = await promiseToast(apiCall(), {
        ...
        success: `Book number ${book_id} deleted successfully!`,
        error: `Failed to delete book ${book_id}`,
      });
      // const result = await promiseToast(apiCall(), {
      //     pending: `Book number ${book_id} deleting...`,
      //     success: {
